...
or slot where a new class, mixin or slot is defined as a subclass of another defined class, mixin or slot.

Note that `is_a` has the characteristics of homeomorphicity.

i.e. `is_a` should be used to define hierarchy between,
...
For example, you cannot define a class as a subclass of mixin or slot.


### abstract

A class may be tagged as `abstract: true` to define that the class is abstract in nature. This has comparable meaning to that in the Object Oriented Programming where another class can use the abstract class as part of its inheritance hierarchy, but the abstract class itself cannot be directly instantiated.

### mixin

The `mixin:true` declaration is used to define a class as a mixin.
...
The `mixins` slot (not to be confused with [`mixin`](#mixin)) can be used to specify a list of mixins that applies to a class. In doing so, any slot that is defined as part of a mixin will be added to the class.
...
The order of the list matters since its a prioritized list where the prefix with the highest priority appears at the top of the list.
...
Unless it is designated as one of the [`defining_slots`](#defining_slots), or [`slot_usage`](#slot_usage) specifies that a given slot is required, then it is **not** mandatory that such a slot is instantiated in all instances of the given class, mixin, or its subclasses.
...
In the `slot_usage` section we define the range and provide a description for the slot's `subject` and `object`, to better document what entities this slot is supposed to link.

### required

The `required` slot can be used to define whether a slot is required.

When a slot is declared as required, via `required: true`, any class that uses that slot must have a value for that slot.
...
The `domain` slot mimics the idea of `rdfs:domain` where you constrain the type of classes that a given slot can be a part of.

### range

The `range` slot mimics the idea of `rdfs:range` where you can constrain the type of classes (or data types) a given slot can have as its value.

You can constrain the range to a specific class, enum, or type.
...
What does it mean for an external concept to be one of `related_mappings`, `broad_mappings`, `narrow_mappings`, `close_mappings`, `exact_mappings`?

Here is an decision tree that will help you determine the granularity of your mappings:

- An external concept can be considered as an exact mapping to class or slot if the former can be used interchangeably with the latter. That is, the semantics are identical and any transitive property that the external concept might bring into
...
- If an external concept can be treated a sub-class of the class or slot then it can be treated as a narrow mapping
- If an external concept can be treated as a super-class of the class or slot then it can be treated as a broad mapping
- If an external concept is distantly related to a class or slot then it can be treated as a related mapping
